...
public class LightLocalizer {
	...
	public static void findRightLine() {
		...
		while(true) {
			...
			if((newColorLeft[0]) <= Navigation.LIGHT_INTENSITY && oldSampleLeft > Navigation.LIGHT_INTENSITY && foundLeft == 0) {
				leftMotor.stop(true);
				foundLeft++;
			}
			// If line detected for right sensor (intensity less than 0.3), only count once by keeping track of last value
			if((newColorRight[0]) <= Navigation.LIGHT_INTENSITY && oldSampleRight > Navigation.LIGHT_INTENSITY && foundRight == 0) {
